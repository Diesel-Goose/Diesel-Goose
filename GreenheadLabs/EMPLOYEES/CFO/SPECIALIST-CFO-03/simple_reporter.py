...
def analyze_real_trades():
                ...
                if len(parts) >= 5:
                    try:
                        all_trades.append({
                            ...
                            'amount': float(parts[3]),
                            'price': float(parts[4]),
                            'pnl': 0.0  # Continuous log doesn't have PNL
                        })
                    except (ValueError, IndexError):
                        continue
    ...
    if len(all_trades) >= 2:
        try:
            ...
            trades_per_hour = total_trades / 24  # Assume 24h if parsing fails
    else:
        trades_per_hour = 0
    ...
    return {
        ...
        'total_pnl_usd': total_pnl * 2.50,  # Approx XRP price
